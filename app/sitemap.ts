...
import { GIFT_GUIDES } from "@/lib/gifts";
...
export default function sitemap(): MetadataRoute.Sitemap {
  const staticRoutes = ["", "/quiz", "/style", "/looks", "/trends", "/collections", "/gifts", "/kits", "/search", "/about", "/contact", "/disclosure"].map((p) => ({
    url: `${BASE}${p}`,
    changeFrequency: "weekly" as const,
    priority: p === "" ? 1 : 0.8,
  }));
  const collectionRoutes = COLLECTIONS.map((c) => ({ url: `${BASE}/collections/${c.slug}`, changeFrequency: "weekly" as const, priority: 0.7 }));
  const giftRoutes = GIFT_GUIDES.map((g) => ({ url: `${BASE}/gifts/${g.slug}`, changeFrequency: "weekly" as const, priority: 0.6 }));
  ...
  return [...staticRoutes, ...collectionRoutes, ...giftRoutes, ...trendRoutes, ...bundleRoutes];
}
